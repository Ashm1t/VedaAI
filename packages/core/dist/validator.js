export function validateLatex(tex) {
    ...
    if (braceBalance !== 0) {
        errors.push(`Unbalanced braces: ${braceBalance > 0 ? braceBalance + " unclosed {" : Math.abs(braceBalance) + " extra }"}`);
    }
    ...
    if (imgMatches) {
        for (const match of imgMatches) {
            ...
            if (fileMatch && fileMatch[1]) {
                warnings.push(`Image reference "${fileMatch[1]}" — file may not exist, could cause compilation warning`);
            }
        }
    }
 ...
 */
function checkEnvironments(tex) {
    const errors = [];
    ...
    for (const env of Object.keys(begins)) {
        ...
        if (bCount !== eCount) {
            errors.push(`Environment "${env}": ${bCount} \\begin vs ${eCount} \\end`);
        }
    }
...
export function autoFixLatex(tex) {
    ...
    fixed = fixed.replace(/\\includegraphics\s*(\[.*?\])?\s*\{[^}]*\}/g, "");
    fixed = fixed.replace(/\\img\{[^}]*\}\{[^}]*\}\{[^}]*\}\{[^}]*\}\{[^}]*\}/g, "");
    fixed = fixed.replace(/\\imgtwo\{[^}]*\}\{[^}]*\}\{[^}]*\}\{[^}]*\}\{[^}]*\}\{[^}]*\}\{[^}]*\}\{[^}]*\}/g, "");
    // Remove empty figure environments left behind
    fixed = fixed.replace(/\\begin\{figure\}.*?\\end\{figure\}/gs, "");
