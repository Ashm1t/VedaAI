 ...
 */
export function buildAnalysisPrompt(extractedText, formData) {
 ...
 */
export function buildSectionPrompt(sectionLabel, questionType, questionTypeLabel, numberOfQuestions, marksPerQuestion, keyConcepts, keyTerms, difficultyGuide, subject, className, additionalInstructions, startingQuestionNumber) {
...
function getDifficultyDistribution(count) {
...
function getSectionInstruction(questionType, marks) {
    switch (questionType) {
        case "mcq":
...
function getQuestionTypeGuidance(type, marks) {
    switch (type) {
        case "mcq":
...
export function selectTemplate(formData) {
 ...
 */
export function computeStartingNumbers(questionTypes) {
    const starts = [];
    let current = 1;
