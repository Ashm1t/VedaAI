...
import { buildAnalysisPrompt, buildSectionPrompt, computeStartingNumbers, selectTemplate, } from "./prompts/template-selection.js";
import { buildLatexGenerationPrompt } from "./prompts/latex-generation.js";
async function analyzeContent(llm, extractedText, formData) {
...
// ── Section Generation ─────────────────────────────────────────────
async function generateSections(llm, formData, analysis) {
    ...
    const sectionPromises = formData.questionTypes.map(async (qt, i) => {
        const prompt = buildSectionPrompt(labels[i] || String(i + 1), qt.type, qt.label, qt.numberOfQuestions, qt.marksPerQuestion, analysis.keyConcepts, analysis.keyTerms, analysis.difficultyGuide, analysis.subject || formData.subject, formData.className, formData.additionalInstructions, startingNumbers[i]);
        try {
            const raw = await llm.generateJSON(prompt);
...
// ── LaTeX Generation ───────────────────────────────────────────────
async function generateLatex(llm, registry, selectedTemplate, questionData) {
    ...
    const prompt = buildLatexGenerationPrompt(templateSource, handbook, questionData, legacyName);
...
async function fixLatex(llm, brokenLatex, errorLog, templateName) {
 ...
 */
export async function runPipeline(llm, registry, extractedText, formData, options, onProgress) {
    ...
    const totalMarks = formData.questionTypes.reduce((sum, qt) => sum + qt.numberOfQuestions * qt.marksPerQuestion, 0);
    const actualMarks = validSections.reduce((sum, s) => sum + s.questions.reduce((qsum, q) => qsum + q.marks, 0), 0);
    if (actualMarks !== totalMarks) {
        console.warn(`Marks mismatch: expected ${totalMarks}, got ${actualMarks}. Using actual total.`);
    }
    ...
    if (!validation.valid) {
        ...
        texContent = await fixLatex(llm, texContent, `Pre-compilation validation errors:\n${validation.errors.join("\n")}`, legacyName);
        texContent = autoFixLatex(texContent);
    }
    ...
    let pdfPath;
    try {
        pdfPath = await compileLaTeX(texContent, options.outputDir, options.documentId, options.latexTimeout);
    }
    catch (err) {
        const errorLog = extractErrorFromLog(err instanceof Error ? err.message : String(err));
        try {
            emit("processing", 85);
